...
def GetClearIni():
    ...
    with open(path,encoding='utf-8')  as f:
        ...
        for i in arr:
            tmp = []
            ft = open('clear_0.txt', 'a', encoding='utf-8')
            # 按照菜名词库匹配
            if len(arr[i]) == 1:
                # 常规菜名
                obj = {}
...
def GetClear():
    with open(path, encoding='utf-8')  as f:
        data = json.load(f)
        with open('clear_0.txt','r',encoding='utf-8') as f2:
            f_new = open('闽菜_0.json','w',encoding='utf-8')
            datas = f2.readlines()
            res = []
            for i in range(0,len(data)):
                ...
                # print(list_cut)
                data[i]['name'] = re.sub(r'[a-zA-Z]|的做法|独家|和配方', '', data[i]['name'])
